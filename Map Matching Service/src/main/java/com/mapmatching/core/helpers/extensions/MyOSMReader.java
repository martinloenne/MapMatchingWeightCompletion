...
// NB: This class is a copy of the class OSMReader from the GraphHopper library
// in order to make a single method public instead of private IN ORDER to override that method
 ...
 * {@link #osmNodeIdToInternalNodeMap}. If a node occurs once it is a pillar
 * node and if more it is a tower node, otherwise
 ...
 * {@link #osmWayIdToRouteWeightMap} for all the ways of the relation.
 * <p>
 ...
 * <p>
 * 2.b) Reads ways from OSM file and creates edges while calculating the speed
 * etc from the OSM tags. When creating an edge the pillar node information from
 * the intermediate data structure will be stored in the way geometry of that
 * edge.
 * <p>
 *
 * @author Peter Karich
 */
public class MyOSMReader implements DataReader, TurnCostParser.ExternalInternalMap {
    protected static final int EMPTY_NODE = -1;
    // pillar node is >= 3
    protected static final int PILLAR_NODE = 1;
    // tower node is <= -3
    protected static final int TOWER_NODE = -2;
    private static final Logger LOGGER = LoggerFactory.getLogger(MyOSMReader.class);
    ...
    // Using the correct Map<Long, Integer> is hard. We need a memory efficient and
    // fast solution for big data sets!
    //
    ...
    // same here: not applicable as ways introduces the nodes in 'wrong' order: new
    // OSMIDSegmentedMap
    ...
    // stores osm way ids used by relations to identify which edge ids needs to be
    // mapped later
    ...
    /**
     * Preprocessing of OSM file to select nodes which are used for highways. This
     * allows a more compact graph data structure.
     */
    ...
    protected void processWay(ReaderWay way) {
        ...
        // TODO move this after we have created the edge and know the coordinates =>
        // encodingManager.applyWayTags
        ...
        if (way.getTag("duration") != null) {
            try {
                ...
                // Provide the duration value in seconds in an artificial graphhopper specific
                // tag:
    ...
    /**
     * @return OSM way ID from specified edgeId. Only previously stored OSM-way-IDs
     *         are returned in order to reduce memory overhead.
     */
    @Override
    ...
    // TODO remove this ugly stuff via better preprocessing phase! E.g. putting
    ...
    double getTmpLatitude(int id) {
        ...
        if (id < TOWER_NODE) {
            ...
            // e.g. if id is not handled from preprocessing (e.g. was ignored via
            // isInBounds)
            return Double.NaN;
    }
    ...
    /**
     * The nodeFlags store the encoders to check for accessibility in edgeFlags.
     * E.g. if nodeFlags==3, then the accessibility of the first two encoders will
     * be check in edgeFlags
     */
    ...
    /**
     * This method creates from an OSM way (via the osm ids) one or more edges in
     * the graph.
     */
        ...
        try {
            for (int i = 0; i < osmNodeIds.size(); i++) {
                ...
                if (tmpNode == PILLAR_NODE) {
                    ...
                    // => if the node before was a pillar node then convert into to tower node (as
                    // it is also end-standing).
                ...
                if (tmpNode < TOWER_NODE) {
                    ...
                    if (firstNode >= 0 && firstNode == tmpNode) {
                        // loop detected. See #1525 and #1533. Insert last OSM ID as tower node. Do this
                        // for all loops so that users can manipulate loops later arbitrarily.
                        long lastOsmNodeId = osmNodeIds.get(i - 1);
                        int lastGHNodeId = getNodeMap().get(lastOsmNodeId);
                        if (lastGHNodeId < TOWER_NODE) {
                            LOGGER.warn("Pillar node " + lastOsmNodeId
                                    + " is already a tower node and used in loop, see #1533. " + "Fix mapping for way "
                                    + wayOsmId + ", nodes:" + osmNodeIds);
                            break;
                        }
    ...
    private int handlePillarNode(int tmpNode, long osmId, PointList pointList, boolean convertToTowerNode) {
        ...
        if (lat == Double.MAX_VALUE || lon == Double.MAX_VALUE)
            throw new RuntimeException("Conversion pillarNode to towerNode already happened!? " + "osmId:" + osmId
                    + " pillarIndex:" + tmpNode);
    ...
    @Override
    public MyOSMReader setWayPointMaxDistance(double maxDist) {
    ...
    @Override
    public MyOSMReader setWorkerThreads(int numOfWorkers) {
        this.workerThreads = numOfWorkers;
        return this;
    }

    @Override
    public MyOSMReader setElevationProvider(ElevationProvider eleProvider) {
        if (eleProvider == null)
            throw new IllegalStateException(
    ...
    private void printInfo(String str) {
        LOGGER.info("finished " + str + " processing." + " nodes: " + graph.getNodes() + ", osmIdMap.size:"
                ...
                + getNodeFlagsMap().size() + ", relFlagsMap.size:" + getRelFlagsMapSize() + ", zeroCounter:"
